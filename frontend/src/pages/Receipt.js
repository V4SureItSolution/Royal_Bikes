...
export const Receipt = () => {
  ...
  const [receipts, setReceipts] = useState([
    { id: 1, receipt_no: '04698', account_code: '2917', customer_name: 'KEERTHANA', receipt_date: '12-08-2026', amount: 4000, payment_type: 'CASH', note: '-', status: 'active' }
  ]);
  ...
  const [formData, setFormData] = useState({ account_code: '', customer_name: '', receipt_date: '12-08-2026', amount: '0', payment_type: '', note: '' });
  const [filters, setFilters] = useState({ fromDate: '12-08-2026', toDate: '12-08-2026' });
  ...
  const handleClear = () => setFormData({ account_code: '', customer_name: '', receipt_date: '12-08-2026', amount: '0', payment_type: '', note: '' });
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSubmitEntry} style={{ maxWidth: '750px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div className="form-actions-row" style={{ justifyContent: 'center', marginTop: '1rem' }}>
              <button type="submit" className="btn-save-pill" style={{ minWidth: '120px' }}>Submit</button>
              <button type="button" onClick={handleClear} className="btn-clear-link">Cancel</button>
            </div>
          </div>
        </form>
      )}
              ...
              <table className="karoda-table">
                <thead>
                  <tr>
                    ...
                    <th>CUSTOMER NAME</th><th>AMOUNT</th><th>NOTE</th><th>STATUS</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedReceipts.length > 0 ? paginatedReceipts.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>
                        <button type="button" className="btn-icon-circle" onClick={() => handleDeleteReceipt(row.id)} style={{ border: '1px solid #e2e8f0' }}>
      ...
      {selectedReceiptForPrint && (
        <>
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.3)', zIndex: 998 }} onClick={() => setSelectedReceiptForPrint(null)} />
          <div style={{ position: 'fixed', top: 0, right: 0, height: '100vh', width: '520px', backgroundColor: '#ffffff', zIndex: 999, boxShadow: '-4px 0 24px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', borderBottom: '1px solid #e2e8f0' }}>
              <div style={{ fontWeight: 700, fontSize: '1rem', color: '#0f172a' }}>Receipt Preview - #{selectedReceiptForPrint.receipt_no}</div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem' }}>
              <div className="printable-receipt-container">
                ...
                <div style={{ borderTop: '1px solid #888', margin: '0.9rem 0 1.25rem' }} />

                <div style={{ fontFamily: "'Times New Roman', serif" }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                    <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>VOUCHER NO: {selectedReceiptForPrint.receipt_no}</div>
                    <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>Date : {(selectedReceiptForPrint.receipt_date || '12-08-2026').replace(/-/g, '/')}</div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', marginBottom: '1.35rem', fontSize: '1.15rem' }}>
                    ...
                    <span style={{ borderBottom: '1px dotted #000', flex: 1, marginLeft: '0.5rem', fontWeight: 'bold', fontSize: '1.25rem', paddingLeft: '0.5rem' }}>{selectedReceiptForPrint.customer_name}</span>
                  </div>

                  <div style={{ display: 'flex', gap: '2rem', marginBottom: '1.35rem', fontSize: '1.15rem' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', flex: 1.2 }}>
                      <span style={{ fontWeight: 'bold', whiteSpace: 'nowrap' }}>Amount :</span>
                      <span style={{ borderBottom: '1px dotted #000', flex: 1, marginLeft: '0.5rem', fontWeight: 'bold', fontSize: '1.25rem', paddingLeft: '0.5rem' }}>{selectedReceiptForPrint.amount}/-</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'baseline', flex: 1 }}>
                      <span style={{ fontWeight: 'bold', whiteSpace: 'nowrap' }}>A/c No :</span>
                      <span style={{ borderBottom: '1px dotted #000', flex: 1, marginLeft: '0.5rem', fontWeight: 'bold', fontSize: '1.25rem', paddingLeft: '0.5rem' }}>{selectedReceiptForPrint.account_code || '2917'}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', marginBottom: '1.5rem', fontSize: '1.15rem' }}>
                    ...
                    <span style={{ borderBottom: '1px dotted #000', flex: 1, marginLeft: '0.5rem', fontWeight: 'bold', fontSize: '1.2rem', paddingLeft: '0.5rem', textTransform: 'lowercase' }}>{numberToWords(selectedReceiptForPrint.amount)}</span>
                  </div>
                  ...
                  <div style={{ marginBottom: '2.75rem' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '1rem', border: '1px solid #666', borderRadius: '20px', padding: '0.35rem 1.4rem', fontSize: '1.05rem' }}>
                      ...
                      <strong>{selectedReceiptForPrint.payment_type || 'CASH'}</strong>
                    </div>
                  </div>

                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '3.5rem', padding: '0 0.5rem' }}>
                    <div style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>Authorised Signature</div>
                    <div style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>Customer Signature</div>
                  </div>

                  <div style={{ textAlign: 'center', fontSize: '0.78rem', color: '#222', marginTop: '1.5rem' }}>
